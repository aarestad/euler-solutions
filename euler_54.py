...
class HandScore:
    ...
    def __repr__(self):
        return "HandScore(%s, %s)" % (
            hand_type_names[self.hand_type],
            self.relevant_ranks,
        )
    ...
    if len(rank_counts) == 2:  # either 4, 1 or 3, 2
        for rank in rank_counts:
            if rank_counts[rank] == 3:
                ...
                return HandScore(
                    HandType.FullHouse, [rank, rank, rank, other_rank, other_rank]
                )
            if rank_counts[rank] == 4:
                other_rank = [r for r in card_ranks if r != rank][0]
                return HandScore(
                    HandType.FourOfAKind, [rank, rank, rank, rank, other_rank]
                )
            ...
            if rank_counts[rank] == 2:
                ...
                for rank2 in rank_counts:
                    if rank_counts[rank2] == 2:
                        other_rank = [
                            r for r in card_ranks if r != rank and r != rank2
                        ][0]
