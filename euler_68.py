...
def magic_5_gon(nums):
	...
	if line1_sum == sum(line2)  and \
		line1_sum == sum(line3) and \
		line1_sum == sum(line4) and \
		line1_sum == sum(line5):

		beginning_line = sorted([line1, line2, line3, line4, line5], \
			lambda x, y: cmp(x[0], y[0]))[0]

		if beginning_line == line1:
			return ''.join([''.join([str(d) for d in line]) for line in [line1, line2, line3, line4, line5]])
		if beginning_line == line2:
			return ''.join([''.join([str(d) for d in line]) for line in [line2, line3, line4, line5, line1]])
		if beginning_line == line3:
			return ''.join([''.join([str(d) for d in line]) for line in [line3, line4, line5, line1, line2]])
		if beginning_line == line4:
			return ''.join([''.join([str(d) for d in line]) for line in [line4, line5, line1, line2, line3]])
		if beginning_line == line5:
			return ''.join([''.join([str(d) for d in line]) for line in [line5, line1, line2, line3, line4]])
